...
The underlying Kubernetes cluster and configuration of the virtual machines themselves is automated, and results in certificate-based IPsec transport between all nodes, and certificate-auth secured etcd.
...
1. storage.yml: This playbook labels the kubernetes nodes as storage nodes, creates a daemonset for glusterfs, and creates a Heketi cluster for vlume management using the topology generated by the up.py script run initially.
